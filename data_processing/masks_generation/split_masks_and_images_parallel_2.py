...
input_path = "/home/srodriguezr2/srodriguezr2_2/retina/Los_Sentinels/data/sentinel2_supercategories/complete"
output_path = "/home/srodriguezr2/srodriguezr2_2/retina/Los_Sentinels/data/sentinel2_supercategories/processed_organized"
    ...
    for filename in os.listdir(period_image_dir):
        ...
        try:
            ...
            for idx, (img_part, mask_part) in enumerate(zip([img_1, img_2], [mask_1, mask_2]), start=1):
                save_dir_img = os.path.join(output_path, f"fold{idx}", "images")
                save_dir_mask = os.path.join(output_path, f"fold{idx}", "masks")

                os.makedirs(save_dir_img, exist_ok=True)
                os.makedirs(save_dir_mask, exist_ok=True)

                img_filename = f"{period_name}_zona_{zone_id}_part_{idx}_image.tif"
                mask_filename = f"{period_name}_zona_{zone_id}_part_{idx}_mask.tif"

                img_save_path = os.path.join(save_dir_img, img_filename)
                mask_save_path = os.path.join(save_dir_mask, mask_filename)
